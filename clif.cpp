...
void clif_parse_BankOpen(int fd, struct map_session_data* sd) {
	...
	if(mapdata->flag[MF_NOBANK]) {
		clif_messagecolor(&sd->bl, color_table[COLOR_RED], "Banking is not allowed in this map.", false, SELF);
		return;
	}
...
void clif_parse_Mail_getattach( int fd, struct map_session_data *sd ){
	...
	if( map_getmapflag(sd->bl.m, MF_NOMAILATTACH) ) {
		clif_messagecolor(&sd->bl, color_table[COLOR_RED], "Receiving mail attachment is disabled on this map!", false, SELF);
		return;
	}
