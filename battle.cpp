	...
	if( sc && sc->count ) { // SC_* that reduce damage to 0.
		...
		if( damage > 0 && (sce = sc->data[SC_STONEHARDSKIN]) ) {
			if( src->type == BL_MOB ) //using explicit call instead break_equip for duration
				...
				skill_break_equip(src,src, EQP_WEAPON, 20, BCT_SELF); //Gidz Mod for Hardskin formula change to 20 from 3000
		}
...
static int battle_calc_attack_skill_ratio(struct Damage* wd, struct block_list *src,struct block_list *target,uint16 skill_id,uint16 skill_lv)
{
	...
	switch(skill_id) {
		...
		case SC_TRIANGLESHOT:
			skillratio += 200 + (skill_lv - 1) * status_get_dex(src) / 2; //Gidz Mod made agi into dex
			RE_LVL_DMOD(120);
			break;
		case SC_FEINTBOMB:
...
static void battle_calc_weapon_final_atk_modifiers(struct Damage* wd, struct block_list *src,struct block_list *target,uint16 skill_id,uint16 skill_lv)
{
	...
	if( tsc && tsc->data[SC_CRESCENTELBOW] && wd->flag&BF_SHORT && rnd()%100 < tsc->data[SC_CRESCENTELBOW]->val2 ) {
		...
		if (ratio > 50) ratio = 50; //Gidz Mod for Crescent Elbow change 5000 into 50
		...
		rdamage = ((int64)rdamage / 5); //Gidz Mod for Crescent Elbow total Reflect Damage Divided by 5
		...
		clif_skill_damage(target, src, gettick(), status_get_amotion(src), 0, rdamage, 1, SR_CRESCENTELBOW_AUTOSPELL, tsc->data[SC_CRESCENTELBOW]->val1, DMG_SINGLE); // This is how official does
				...
				switch(skill_id) {
					...
					case MG_LIGHTNINGBOLT:
						if (sc && sc->data[SC_SPELLFIST] && mflag&BF_SHORT)  {
							skillratio += (sc->data[SC_SPELLFIST]->val4 * 5) + (sc->data[SC_SPELLFIST]->val1 * 20) - 100;// val4 = used bolt level, val2 = used spellfist level. [Rytech] // Gidz Mod to Nerf Spell Fist
			...
			if( sc->data[SC_REFLECTDAMAGE] && !skill_get_inf2(skill_id, INF2_ISTRAP)) {
				...
				if (sc->data[SC_DEATHBOUND] && skill_id != WS_CARTTERMINATION && skill_id != GN_HELLS_PLANT_ATK && !status_bl_has_mode(src,MD_STATUS_IMMUNE)) {
					if (distance_bl(src,bl) <= 0 || !map_check_dir(map_calc_dir(bl,src->x,src->y), unit_getdir(bl))) {
						...
						*dmg = rd1 * 10 / 100; // Received damage = 30% of amplified damage. [Gidz Mod change to 30% to 10% 255/120 setup]
						...
						rdamage += rd1 * 5 / 100; // Target receives 70% of the amplified damage. [Rytech] [Gidz Mod change to 70% to 5% 255/120 setup]
					}
				}
...
int battle_check_target( struct block_list *src, struct block_list *target,int flag)
{
	...
	{	//Checks on source master
		case BL_PC:
		{
			...
			if( s_bl != t_bl )
			{
				...
				else if (sd->duel_group && !((!battle_config.duel_allow_pvp && mapdata->flag[MF_PVP]) || (!battle_config.duel_allow_gvg && mapdata_flag_gvg(mapdata)))) // Gidz Mod @pk command
				{
	...
	{ //Check rivalry settings.
		...
		if( flag&(BCT_GUILD|BCT_ENEMY) )
		{
			...
			if( !(mapdata->flag[MF_PVP] && mapdata->flag[MF_PVP_NOGUILD]) && s_guild && t_guild && (s_guild == t_guild || (!(flag&BCT_SAMEGUILD) && !mapdata->flag[MF_GVG_NOALLY] && guild_isallied(s_guild, t_guild))) && (!mapdata->flag[MF_BATTLEGROUND] || sbg_id == tbg_id) ) // Gidz Mod for Alliance
				state |= BCT_GUILD;
			else
				state |= BCT_ENEMY;
		}
...
// rAthena
	...
	{ "reserved_costume_id",				&battle_config.reserved_costume_id,				999998,	0,		INT_MAX,		}, // Gidz Mod for Costume
	...
	{ "request_delay",						&battle_config.request_delay,					0,      0,      INT_MAX,        }, //@request delay
	{ "command_delay",						&battle_config.command_delay,					0,      0,      INT_MAX,        }, //Global @command delay
	// Market Clone [AnnieRuru/Dastgir]
